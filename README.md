# MK Lounge Static Scorer

MK8DXラウンジ向けの完全静的な集計アプリです。

この版ではサーバーを使いません。管理UIとOBS用overlay表示を同じページ内に置き、OBS側でoverlay部分をクロップして使う方針です。

## MVP方針

- 1レース目はOCRせず、ユーザーが完全に手入力する
- 2レース目以降はブラウザ内で自動判定する
- 集計結果は同じ画面内のoverlay previewに表示する
- OBSではoverlay preview部分だけをクロップする
- 状態はブラウザのlocalStorage / IndexedDBに保存する
- サーバー、DB、WebSocketは使わない

## 現在できること

Milestone 1:

- 1レース目以降の手入力
- 順位別点数の自動計算
- チーム別集計
- 既存bot overlayデザインを流用した管理画面内overlay preview
- 自チーム強調表示
- Race History編集
- localStorage保存
- Reset All

Milestone 2:

- `getDisplayMedia` による画面共有
- videoからcanvasへのフレーム描画
- 1280x720基準の順位テンプレート読み込み
- 1〜12位のrank score算出
- リザルト画面らしさの判定
- 判定スコアの画面表示
- リザルト判定されたスクリーンショットの表示
- 最初に検出したリザルトを上書きしない制御
- レース処理後の連続検出防止

Milestone 3:

- 検出フレームから2レース目以降の自動判定に必要なデータを作成
- 1レース目の手入力結果とリザルト検出が揃った時点で自動判定の準備を完了
- 2レース目以降の検出結果をユーザー確認用に表示
- 候補をRace Historyへ追加

Milestone 4:

- Race Historyの編集
- 編集後の即時再集計
- レース削除
- 1レース目削除時の自動判定データ破棄
- 保存データversion管理
- ページ再読み込み後の復元
- Reset Allによる保存データ削除

## ローカルで開く

ビルドは不要です。画面共有とテンプレート画像読み込みを使うため、ローカルHTTPサーバー経由で開くのがおすすめです。

```bash
python3 -m http.server 8000
```

その後、以下を開きます。

```text
http://localhost:8000/
```

## OBSでの使い方

1. ブラウザでアプリを開く
2. 管理UIでレース結果を入力する
3. OBSでブラウザ画面をキャプチャする
4. 右側の `Overlay Preview` 部分だけをクロップする

画面共有を開始すると、リザルト画面を自動で監視します。リザルト画面を検出すると、そのスクリーンショットを表示し、確認待ちになります。1レース目入力とリザルト検出が揃うと、2レース目以降の自動判定準備が完了します。その後は同じリザルトを重複処理しないよう、しばらく次のレースを待ちます。

詳しい使い方は [docs/USAGE.md](./docs/USAGE.md) を参照してください。

## デプロイ

GitHub Pages / Cloudflare Pagesで配信できます。ビルドは不要です。

詳しい手順は [DEPLOYMENT.md](./DEPLOYMENT.md) を参照してください。

## テスト

```bash
python3 -m pytest -q
```
